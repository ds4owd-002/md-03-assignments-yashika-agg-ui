...
```{r}
penguins |>
  filter(species != "chinstraps", bill_length_mm > 40)
```
...
```{r}
penguins |>
  filter(species== "Adelie", sex== "female", island %in% c("Dream", "Torgersen"))
```
...
1.  What new object appears in your Environment?
    ...
    **Ans:** Penguins flipper length sum appears grouped by species.

2.  What is the difference between this code and the previous code chunks?

    **Ans:** Summarizes the data by species (one row per group), whereas the previous code chunks filtered or mutated data without reducing the number of rows

3.  Why would we use the assignment operator `<-` to save our results?

    **Ans:** To store the result in an object so you can reuse it later without rerunning the code.
