...
```{r}
sanitation_ind <- sanitation |> 
  filter(iso3 == "IND",
         year %in% c(2000, 2020), 
         varname_short != "san_in")
```
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend?
    ...
    ans: he order of the bars and the legend follows the default factor level (or alphabetical) order of `varname_long`, rather than a meaningful or logical order.

2.  What would you want to change?

    Ans: would reorder `varname_long` so the bars and legend reflect a meaningful progression of sanitation categories rather than the default ordering.

3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?

    Ans: We removed “safely managed sanitation services” because it overlaps with “basic sanitation services,” so including both would double count and distort the stacked proportions.
